...
- `Vec` stays restricted to `&mut Self` accessors until regions land.
  A container you can only read through a unique borrow is not the final
  API, but it is sound and it unblocks the bootstrap.
