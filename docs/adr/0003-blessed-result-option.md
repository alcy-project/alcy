# ADR-0003: Deferred Generics with Blessed Result and Optional
...
The error model needs `Result<T, E>` and `Optional<T>`, which are
...
- `Result<T, E>` and `Optional<T>` are compiler-known, monomorphized
