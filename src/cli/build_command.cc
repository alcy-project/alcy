...
bool emit_package_object(CliContext& ctx,
                         lower::LoweredPackage& package,
                         bool optimize,
                         const std::string& output_path) {
  llvm::LLVMContext context;
  (void)optimize;  // cli layer writes file; lib returns memory buffer
...
bool link_executable(CliContext& ctx,
  ...
  if (linked.is_err()) {
    const u32 index = ctx.bag.emit(diag::Severity::Error, kCliLinkError,
...
ResultCode build_single_file(CliContext& ctx,
...
ResultCode build_package(CliContext& ctx,
...
ResultCode run_build(const CliConfig& config) {
  ...
  const u32 index = ctx.bag.emit(
      diag::Severity::Warning, kCliNoManifest,
